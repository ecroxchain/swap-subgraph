...
export function handleSync(event: Sync): void {
  ...
  if (token0) {
    ...
    token0.derivedUSD = t0DerivedBNB.times(bundle!.bnbPrice || BigDecimal.fromString("0"));
    token0.save();
  }
  ...
  if (token1) {
    ...
    token1.derivedUSD = t1DerivedBNB.times(bundle!.bnbPrice || BigDecimal.fromString("0"));
    token1.save();
  }
  ...
  if (pair) {
    ...
    pair.reserveBNB = pair.reserve0
      .times((token0!.derivedBNB || BigDecimal.fromString("0")) as BigDecimal)
      .plus(pair.reserve1.times((token1!.derivedBNB || BigDecimal.fromString("0")) as BigDecimal));
    pair.reserveUSD = pair.reserveBNB.times(bundle!.bnbPrice || BigDecimal.fromString("0"));
  }
  ...
  if (token0) {
    ...
    token0.totalLiquidity = token0.totalLiquidity.plus(pair!.reserve0 || BigDecimal.fromString("0"));
  }
  if (token1) {
    // now correctly set liquidity amounts for each token
    token1.totalLiquidity = token1.totalLiquidity.plus(pair!.reserve1 || BigDecimal.fromString("0"));
  }
...
export function handleMint(event: Mint): void {
  ...
  let token0Amount = BigDecimal.fromString("0");
  let token1Amount = BigDecimal.fromString("0");
  if (token0) {
  ...
  if (token1) {
    // update exchange info (except balances, sync will cover that)
    token1Amount = convertTokenToDecimal(event.params.amount1, token1.decimals);
  ...
  let amountTotalUSD =
    token1!
      ...
      .times(bundle!.bnbPrice || BigDecimal.fromString("0")) || BigDecimal.fromString("0");
...
export function handleBurn(event: Burn): void {
  ...
  let amountTotalUSD =
    token1!
      .derivedBNB!.times(token1Amount)
      .plus(token0!.derivedBNB!.times(token0Amount) || BigDecimal.fromString("0"))
      .times(bundle!.bnbPrice || BigDecimal.fromString("0")) || BigDecimal.fromString("0");
...
export function handleSwap(event: Swap): void {
  ...
  let token0 = Token.load(pair!.token0);
  let token1 = Token.load(pair!.token1);
  let amount0In = BigDecimal.fromString("0");
  let amount1In = BigDecimal.fromString("0");
  let amount0Out = BigDecimal.fromString("0");
  let amount1Out = BigDecimal.fromString("0");
  ...
  let derivedAmountBNB =
    token1!
      .derivedBNB!.times(amount1Total)
      .plus(token0!.derivedBNB!.times(amount0Total) || BigDecimal.fromString("0"))
      .div(BigDecimal.fromString("2")) || BigDecimal.fromString("0");
  let derivedAmountUSD = derivedAmountBNB.times(bundle!.bnbPrice || BigDecimal.fromString("0"));
  ...
  if (pancake) {
    ...
    pancake.totalVolumeBNB = pancake.totalVolumeBNB.plus(trackedAmountBNB! || BigDecimal.fromString("0"));
  ...
  pancakeDayData.dailyVolumeBNB = pancakeDayData.dailyVolumeBNB.plus(trackedAmountBNB! || BigDecimal.fromString("0"));
  ...
  token0DayData.dailyVolumeBNB = token0DayData.dailyVolumeBNB.plus(
    amount0Total.times((token0!.derivedBNB || BigDecimal.fromString("0")) as BigDecimal)
  );
  token0DayData.dailyVolumeUSD = token0DayData.dailyVolumeUSD.plus(
    amount0Total
      .times((token0!.derivedBNB || BigDecimal.fromString("0")) as BigDecimal)
      .times(bundle!.bnbPrice || BigDecimal.fromString("0"))
  );
  token0DayData.save();
  ...
  token1DayData.dailyVolumeBNB = token1DayData.dailyVolumeBNB.plus(
    amount1Total.times((token1!.derivedBNB || BigDecimal.fromString("0")) as BigDecimal)
  );
  token1DayData.dailyVolumeUSD = token1DayData.dailyVolumeUSD.plus(
    amount1Total
      .times((token1!.derivedBNB || BigDecimal.fromString("0")) as BigDecimal)
      .times(bundle!.bnbPrice || BigDecimal.fromString("0"))
  );
  token1DayData.save();
}
